...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  if (arguments.length == 0) {return 0} 
    ...
    multiplicarE = arguments[i] * arguments[i+1];
    if (multiplicarE >= 0) {multiplicar = multiplicarE}
  }}
return multiplicar
}
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  // este codigo esta mal, perreros;
let allEqual = arreglo
if (arreglo.every (v => v === arreglo[0])) {
  return true 
}
else { return false}
  }
...
arrayNueva.push('Enero');}
else {
  return "No se encontraron los meses pedidos"
}
if (array.includes('Marzo')) {
arrayNueva.push('Marzo');}
else {
  return "No se encontraron los meses pedidos"
}
if (array.includes('Noviembre')) {
arrayNueva.push('Noviembre');}
else {
  return "No se encontraron los meses pedidos"
}
return arrayNueva;
...
function breakStatement(numero) {
  ...
  // Tu código:
  const numero1 = numero
  let i = 0;
  let aumento = 0
  let arrayAumento =[];
  for (let i = 0; i < 10; i++) {
     aumento = numero += 2;
     arrayAumento.push(aumento);
     if (i+1 === aumento) {
